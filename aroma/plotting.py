...
def classification_plot(myinput, outDir):
    ...
    print(
        "Found",
        nmot,
        "head motion-related components in a total of",
        ncomp,
        "components.",
    )
    ...
    # rename columns
    df = df.rename(
        index=str, columns={0: "Motion", 1: "RP", 2: "Edge", 3: "Freq", 4: "CSF"}
    )
    ...
    # plot Freq
    sns.boxplot(x="Motion", y="Freq", data=df, ax=ax2, palette=[colortrue, colorfalse])
    ...
    # plot CSF
    sns.boxplot(x="Motion", y="CSF", data=df, ax=ax3, palette=[colortrue, colorfalse])
    ...
    # outtakes
    plt.savefig(
        os.path.join(outDir, "ICA_AROMA_component_assessment.pdf"), bbox_inches="tight"
    )
